...
namespace StationLinerMVC.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        public JsonResult AllocateDeChannel()
        {
            ...
            if (action == "assign")
            {
                if (!db.UserAllocatedChannels.Any(x => x.ChannelId == channelId && x.UserId == userId))
                {
                    var channel = db.UserAllocatedChannels.Add(
                        new UserAllocatedChannels() {ChannelId = channelId, UserId = userId});
                    db.SaveChanges();
                }
            ...
            {
                var record = db.UserAllocatedChannels.Where(x => x.ChannelId == channelId && x.UserId == userId);
                if (record.Any())
                {
        ...
        public JsonResult CheckAllocation()
        {
            ...
            var userId = int.Parse(Request.Form["UserId"]);
            var channelId = int.Parse(Request.Form["ChannelId"]);
            if (db.UserAllocatedChannels.Any(x => x.UserId == userId && x.ChannelId == channelId))
            {
                success = "allocated";
            }
            return Json(success);
